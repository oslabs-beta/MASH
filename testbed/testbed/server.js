...
io.on('connect', function (socket) {
    ...
    socket.on(socket_1.SocketNames.produceRate, function (data) {
        var topic = data.topic, rate = data.rate;
        producer_1.produceAtRate(topic, rate);
    });
    ...
    socket.on(socket_1.SocketNames.consumeAll, function (data) {
        ...
        consumer_1.consume(socket, topic !== '' ? topic : undefined);
    });
});
var PORT = 2022;
app.get('/main.css', function (req, res) { return res.sendFile(path.resolve(__dirname, 'main.css')); });
app.get('/main.js', function (req, res) { return res.sendFile(path.resolve(__dirname, '../test-front/main.js')); });
